...
The family is a rebrand of **Cascadia Mono** -- the ligature-free cut of Cascadia.
The four RIBBI styles are produced by pinning the `wght` axis of Cascadia Mono's variable fonts,
so the whole family derives from two source files.

```
rnetonet/
  sources/                      <- upstream Cascadia Mono (build inputs)
...
The Regular is pinned at Cascadia's SemiLight named instance (wght 350) and the Bold at the Regular
named instance (wght 400) -- both are named instances of the source, 50 axis units apart, so the
four files bold- and italic-link as one RIBBI family.
...
and `head.flags` keeps its integer-PPEM bit since Cascadia is manually hinted. Cascadia Mono is the
ligature-free cut -- its GSUB carries contextual alternates and stylistic sets (whose UI name labels
are preserved) but none of Cascadia Code's coding ligatures, so `-> => !=` render as plain glyph
sequences.
